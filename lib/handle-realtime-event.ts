...
export default function handleRealtimeEvent(
  ...
  if (ev?.type === "session.info" && Array.isArray(ev.history)) {
    for (const h of ev.history) {
      // replay each historical object into the same handler
      // it's OK to call recursively because each replayed item will hit one of the handled cases
      handleRealtimeEvent(h.event || h, setItems);
    }
    return;
  }
